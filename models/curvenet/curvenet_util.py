...
class LPFA(nn.Module):
    def __init__(self, in_channel, out_channel, args, k, mlp_num=2, initial=False):
...
class CIC(nn.Module):
    def __init__(self, npoint, radius, k, in_channels, output_channels, args, bottleneck_ratio=2, mlp_num=2, curve_config=None):
        ...
        if self.use_curve:
            ...
            self.curvegrouping = CurveGrouping(planes, k, curve_config[0], curve_config[1], args)
        ...
        self.lpfa = LPFA(planes, planes, args, k, mlp_num=mlp_num, initial=False)
...
class CurveGrouping(nn.Module):
    def __init__(self, in_channel, k, curve_num, curve_length, args):
        ...
        self.walk = Walk(in_channel, k, curve_num, curve_length, args)
